...
const About = () => {
  return (
    <section className='min-h-screen w-full text-dimWhite flex flex-col gap-8 md:gap-0 px-2 md:px-20 pb-20 md:pb-0'>
      ...
      <div className="flex justify-between gap-10 mx-5 md:mx-0 md:gap-10 md:px-16 overflow-auto scroll-1 z-10">
        {
          about.map((item, index) => (
            <img className="w-[100px] md:w-[150px]" src={item.src} alt={`image${item.id}`} key={index} />
          ))
        }
      </div>
    </section>
